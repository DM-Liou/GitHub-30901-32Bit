...
def DoJob(Bot, x):
    ...
    for case in switch(x.job_type):
        ...
        if case(Job.DELETESYMBOL):
            ...
            break
    print('5-END. def DoJob(Bot, x):  END')
...
class StockBot:
    print('6-START. class StockBot: START')
...
def run_job():
    ...
    while not q.empty():
        ...
        DoJob(Bot, next_job)
        print('----------------------------------------------------------------------')
        print('4-END. def run_job(): /DoJob(Bot, next_job END  and RETURN TO MainLoop')
        print('----------------------------------------------------------------------')
        print('')
        #----------------------------------------------------------------------------  
#===========================================================================
...
'''
此代碼定義了一個 StockBot 類，它使用帳戶、密碼和 stockframe 對象進行初始化。 它還包含連接、登錄、RegisterQuoteSymbol 和 UnRegisterQuoteSymbol 方法。 connect 方法創建一個 stockapi 對象，設置一個事件處理程序，並連接到服務器。 登錄方法使用提供的帳號和密碼登錄服務器。 RegisterQuoteSymbol 和 UnRegisterQuoteSymbol 方法分別註冊和取消註冊引號。
代碼的主要部分創建了一個 Queue 對象 q 和一個 StockFrame 對象 frame。 然後它使用帳戶、密碼和框架對象創建一個 StockBot 對象 Bot。 最後，它進入一個無限循環，嘗試從 q 對象獲取作業，並以 Bot 對象和作業作為參數調用 DoJob 函數。
'''
